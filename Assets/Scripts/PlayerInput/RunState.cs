...
public class RunState : PlayerBaseState
{
    ...
    void HandleRun()
    {
        float speed = Ctx.IsCrouching ? Ctx.CrouchSpeed : Ctx.SprintSpeed;
        ...
        if (currentinputVertical == 0 && currentinputHorizontal == 0 && !Ctx.IsRunPressed)
        {
        ...
        else if (currentinputVertical != 0 && currentinputHorizontal != 0)
        {
            float minSmoothedVelocityX = -0.5f;
            float maxSmoothedVelocityX = 0.5f;
            float minSmoothedVelocityZ = -0.5f;
            float maxSmoothedVelocityZ = 0.5f;
        ...
        smoothedVelocityX = Mathf.Clamp(smoothedVelocityX, -1f, 1f);
        smoothedVelocityZ = Mathf.Clamp(smoothedVelocityZ, -1f, 1f);
